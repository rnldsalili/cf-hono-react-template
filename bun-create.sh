...
echo ""
echo -e "  ${DIM}1.${RESET} Configure environment variables"
echo -e "     ${DIM}\$${RESET} cp apps/api/.dev.vars.example apps/api/.dev.vars"
echo -e "     ${DIM}\$${RESET} cp apps/admin/.env.example apps/admin/.env"
echo ""
echo -e "  ${DIM}2.${RESET} Create Cloudflare D1 databases and copy the ${BOLD}database_id${RESET} values"
...
echo ""
echo -e "  ${DIM}3.${RESET} Apply DB migrations locally"
echo -e "     ${DIM}\$${RESET} cd apps/api && bunx wrangler d1 migrations apply ${PROJECT_NAME}-dev --local"
echo ""
echo -e "  ${DIM}4.${RESET} Start development servers"
